...
async function loadDataFile(src){
 const r=await fetch(src+'?rev=107c-'+Date.now(),{cache:'no-store'});
 if(!r.ok) throw new Error(src+' returned HTTP '+r.status);
 const raw=await r.text();
 // v1.0.7 content was committed with an accidental extra ] at the end of
 // several add(...) calls: ...}]); instead of ...});. Repair only that exact
 // syntactic sequence before evaluation; event prose and data are untouched.
 const repaired=raw.replace(/\}\]\);/g,'});');
 try{(0,eval)(repaired)}catch(err){throw new Error(src+' syntax error after repair: '+err.message)}
}
async function ensureData(){
 window.PA107_GENERAL=[];window.PA107_PATHS=[];
 await loadDataFile('events-general-107.js');
 await loadDataFile('events-paths-107.js');
 const all=getAll();
 if(all.length!==69) throw new Error('Expected 69 events but loaded '+all.length);
 return all;
}
...
function errorState(where,err){where.innerHTML=`<div class="eventnote" style="border-color:#b45c55"><b>Event data did not load.</b><br>${String(err&&err.message||err||'Unknown error')}<br><small>The Event Bank expected 69 events. This diagnostic is shown instead of a false zero count.</small></div>`}
async function bank(){
 ...
 let all;try{all=await ensureData()}catch(err){console.error(err);errorState(vg,err);return}
...
async function volume(key){
 document.body.insertAdjacentHTML('afterbegin',nav());const v=V[key];document.documentElement.style.setProperty('--accent',v.accent);
 const hero=document.getElementById('volumehero'),main=document.getElementById('volumemain');
 let all;try{all=await ensureData()}catch(err){hero.innerHTML=`<div class="countline">Event Volume ${v.roman} · v1.0.7</div><h1>${v.title}</h1>`;errorState(main,err);return}
